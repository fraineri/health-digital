...
import type { AmaLevel } from "@prisma/client";
import { cn } from "@/lib/utils";

interface AmaIndicatorProps {
  value: AmaLevel | null;
  onChange: (value: AmaLevel) => void;
}
...
const AMA_LEVELS: {
...
}[] = [
  {
    ...
    active: "bg-slate-100 text-slate-500 border-slate-300",
  },
  {
    id: "LOW",
    label: "Leve",
    color: "bg-[#d1dacd]/20 text-[#4d5e4a] border-[#aabda4]/30",
    active: "bg-[#d1dacd] text-[#4d5e4a] border-[#aabda4]",
  },
  {
    id: "MEDIUM",
    label: "Moderada",
    color: "bg-[#f2e1c9]/20 text-[#8c6738] border-[#e6c8a3]/30",
    active: "bg-[#f2e1c9] text-[#8c6738] border-[#e6c8a3]",
  },
  {
    id: "HIGH",
    label: "Severa",
    color: "bg-[#e2c4c4]/20 text-[#8a4242] border-[#d6a5a5]/30",
    active: "bg-[#e2c4c4] text-[#8a4242] border-[#d6a5a5]",
  },
];

export function AmaIndicator({ value, onChange }: AmaIndicatorProps) {
  const currentLevel =
    AMA_LEVELS.find((l) => l.id === value) || AMA_LEVELS[0];
